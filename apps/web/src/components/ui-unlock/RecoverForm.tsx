...
export function RecoverForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setErr(null);
    if (!/^\d{6}$/.test(p1)) {
      setErr("Mã PIN mới phải đúng 6 chữ số");
      return;
    }
    if (p1 !== p2) {
    ...
    try {
      const res = await fetchJSON<{ new_recovery_key: string }>(
        "/api/v1/ui/recover",
        { json: { recovery_key: key, new_pin: p1 } }
      );
      markSession();
  ...
  if (newRecovery) {
    return (
      <RecoveryKeyPanel
        ...
        label="Khoá khôi phục MỚI (khoá cũ đã vô hiệu)"
        onContinue={() => {
          router.push("/");
          router.refresh();
        }}
  ...
  return (
    <form onSubmit={submit} className="card flex flex-col gap-3">
      <label className="flex flex-col gap-1 text-sm">
        <span className="muted">Khoá khôi phục</span>
        <input
          autoFocus
